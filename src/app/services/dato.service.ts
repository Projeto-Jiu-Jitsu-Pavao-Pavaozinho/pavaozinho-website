...
export class DatoService{
  ...
  public getAllBlogPosts(): Observable<any> {
    const query = "{ allPosts(filter: {_status: {eq: published}}) { id, slug, _firstPublishedAt, titulo, corpo, capa{ url } } }"
  ...
  public getPostBySlug(slug: string): Observable<any> {
    const query = `{ post(filter: {slug: {eq: "${slug}"}}) { id, slug, _firstPublishedAt, titulo, corpo, capa{ url } } }`
    return this.http.get<BlogPost[]>(`${this.url}?query=${encodeURIComponent(query)}`, this.httpOptions);
  }
}
